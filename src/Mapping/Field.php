...
class Field
{
	...
	public function mergeAnnotation(string $name, AnnotationField $annotation) {
		...
		$this->setValue($annotation->getValue() ?: $this->getValue());
		$this->setOptions(array_merge_recursive($this->options, $annotation->getOptions()));

		// @TODO : Check the solidity of this feature with usecases
		if ($annotation->getName()) {
			$this->setOptions(array_merge([ 'mapped' => false ], $this->options));
		}
